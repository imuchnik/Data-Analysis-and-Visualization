...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    cum_ret = get_cum_return(port_val)
    daily_ret = get_daily_ret(port_val)
    avg_daily_ret = get_avg_daily_ret(daily_ret)
    std_daily_ret = get_std_daily_ret(daily_ret)
    sharpe_ratio = get_sharpe_ratio(avg_daily_ret, std_daily_ret)
...
def plot_normalized_data(df, title="Normalized prices", xlabel="Date", ylabel="Normalized price"):
    ...
    ----------
        ...
        ylabel: Y-axis label
    """
    data_to_plot = get_normed(df)
    ax=data_to_plot.plot(title=title)
...
def get_std_daily_ret(daily_ret):
    return daily_ret.std()

def get_cum_return(port_val):
    return (port_val[-1]/port_val[0])-1

def get_sharpe_ratio(avg_daily_ret, std_daily_ret):
   return math.sqrt(252) * (avg_daily_ret/std_daily_ret)
...
def assess_portfolio(start_date, end_date, symbols, allocs, start_val=1):
    ...
    daily_ret = get_daily_ret(port_val)
   # print daily_ret
    avg=get_avg_daily_ret(daily_ret)
    #print "AVERAGE",avg
    #avg_mean = avg.mean()
    #print "AVG MEAN:", avg_mean
    std = get_std_daily_ret(daily_ret)
    #print std
    cum = get_cum_return(daily_ret)
    #print cum
    shrp = get_sharpe_ratio(avg, std)
    #print shrp
